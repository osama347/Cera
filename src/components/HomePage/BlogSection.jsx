...
const BlogSection = ({ onBlogAdded }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const blogResponse = await fetch('http://localhost:5000/api/blogs?_sort=createdAt&_order=desc&_limit=6&_=' + new Date().getTime());
      if (!blogResponse.ok) throw new Error('Failed to fetch blogs');
      const blogData = await blogResponse.json();
      // Ensure at least 3 blogs, up to 6
      setBlogs(blogData.slice(0, Math.max(3, Math.min(6, blogData.length))));
    } catch (err) {
